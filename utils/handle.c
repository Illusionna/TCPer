...
void handle_thread_task(void *args) {
    ...
    if (socket_setopt(c, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof(timeout)) == SOCKET_INVALID) {
        asynclog_warning("socket timeout setting failed.");
        socket_close(c);
        return;
    }
    ...
    if (offset >= filesize) {
        ...
        if (strcmp(hash, meta.md5) != 0) {
            ...
            snprintf(
                response, sizeof(response),
                "[FAILURE] the MD5 of server and client dismatch, deleting file \"%s\".",
                os_basename(safe_filename)
            );
            asynclog_warning(response);
            socket_send(c, response, strlen(response), 0);
        } else {
            set_add(set, hash);
            char size[16];
            ...
            snprintf(
                response, sizeof(response),
                "[OK] file \"%s\" has been successfully uploaded:\n"
                " - MD5 hash: %s\n"
                " - File size: %s\n"
                " - Cost time: %.5lf s",
                os_basename(safe_filename), hash, size, os_time() - start_time
            );
            asynclog_info(response);
            socket_send(c, response, strlen(response), 0);
        }
